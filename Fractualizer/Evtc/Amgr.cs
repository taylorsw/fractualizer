...
namespace Evtc
{
    ...
    public class Amgr
    {
        ...
        internal void Update(double dtms)
        {
            ...
            while (lln != null)
            {
                ...
                {
                    Remove(lln);
                    Avar avarNext = avar.dgNext?.Invoke();
                    if (avarNext != null)
                        Add(avarNext);
                }
                else
        ...
        {
            if (avar.kamgt == Kamgt.Fixed && mpavark_avar.ContainsKey(avar.avark))
                return;
    ...
    public class Avark
    {
        public static Avark New() { return new Avark(); }
        private Avark() { }
    }
    ...
    {
        public delegate Avar DgAvarNext();
    ...
    public abstract class Avar<T> : Avar
    {
        ...
        protected Avar(DgReadVal dgInitialValue, DgReadVal dgValDst, DgWriteVal dgWriteVal, T tval = default(T), Avark avark = null,
            Kamgt kamgt = Kamgt.Overridable)
            : base(avark, kamgt)
        {
    ...
    public class AvarLinearDiscrete<T> : Avar<T>
    {
        ...
        public AvarLinearDiscrete(DgReadVal dgReadVal, double valDst, DgWriteVal dgWriteVal, double dtmsPeriod,
            T tval = default(T), Avark avark = null,
            Kamgt kamgt = Kamgt.Overridable)
            : base(dgReadVal, avar => valDst, dgWriteVal, tval, avark, kamgt)
        {
            this.dtmsPeriod = dtmsPeriod;
        }

        public AvarLinearDiscrete(DgReadVal dgReadVal, double valDst, double dval_dtms, DgWriteVal dgWriteVal,
            T tval = default(T), Avark avark = null,
            Kamgt kamgt = Kamgt.Overridable)
            : this(dgReadVal, valDst, dgWriteVal, double.NaN, tval, avark, kamgt)
        {
            this.dval_dtms = dval_dtms;
        }

        public AvarLinearDiscrete(double valInitial, double valDst, double dval_dtms, DgWriteVal dgWriteVal,
            T tval = default(T), Avark avark = null,
            Kamgt kamgt = Kamgt.Overridable)
            : this(avar => valInitial, valDst, dgWriteVal, double.NaN, tval, avark, kamgt)
        {
            this.dval_dtms = dval_dtms;
        }

        public AvarLinearDiscrete(double valInitial, double valDst, DgWriteVal dgWriteVal, double dtmsPeriod,
            T tval = default(T), Avark avark = null,
            Kamgt kamgt = Kamgt.Overridable)
            : this(avar => valInitial, valDst, dgWriteVal, dtmsPeriod, tval, avark, kamgt)
        {
        }

        public static AvarLinearDiscrete<T> BounceBetween(
            DgReadVal dgReadVal,
            DgWriteVal dgWriteVal,
            double valMin,
            double valMax,
            double dval_dtms)
        {
            double dval_dtmsAbs = Math.Abs(dval_dtms);
            var avarBounceForward = new AvarLinearDiscrete<T>(
                dgReadVal: dgReadVal,
                valDst: valMax,
                dgWriteVal: dgWriteVal,
                dval_dtms: dval_dtmsAbs);
            var avarBounceBackwards = new AvarLinearDiscrete<T>(
                dgReadVal: dgReadVal,
                valDst: valMin,
                dgWriteVal: dgWriteVal,
                dval_dtms: -dval_dtmsAbs);
            avarBounceForward.SetDgNext(() => avarBounceBackwards);
            avarBounceBackwards.SetDgNext(() => avarBounceForward);
    ...
    public class AvarLinearIndefinite<T> : Avar<T>
    {
        ...
        public AvarLinearIndefinite(DgReadVal dgReadVal, DgWriteVal dgWriteVal, double dval_dtms, T tval = default(T), Avark avark = null,
            Kamgt kamgt = Kamgt.Overridable)
            : base(dgReadVal, dgReadNan, dgWriteVal, tval, avark, kamgt)
        {
            this.dval_dtms = dval_dtms;
        }

        public AvarLinearIndefinite(double valInitial, DgWriteVal dgWriteVal, double dval_dtms, T tval = default(T),
            Avark avark = null,
            Kamgt kamgt = Kamgt.Overridable)
            : this(avar => valInitial, dgWriteVal, dval_dtms, tval, avark, kamgt)
        {
            this.dval_dtms = dval_dtms;
        }
    ...
    {
        public AvarIndefinite(DgWriteVal dgWriteVal, T tval = default(T),
            Avark avark = null, Kamgt kamgt = Kamgt.Overridable)
            : base(dgReadZero, dgReadNan, dgWriteVal, tval, avark, kamgt)
        {
        }
